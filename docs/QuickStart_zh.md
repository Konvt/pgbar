...
同时在多个线程中尝试 move 或 swap `MultiBar`、或者在某个线程中 move 或 swap 而在其他线程中调用 `MultiBar` 的其他方法，是线程不安全的。
...
此外，同时在多个线程中尝试 move 或 swap `DynamicBar`、或者在某个线程中 move 或 swap 而在其他线程中调用 `DynamicBar` 的其他方法，是线程不安全的。
...
着色效果的自动关闭与否可以由 `pgbar::config::auto_style_off()` 函数开启或关闭。
